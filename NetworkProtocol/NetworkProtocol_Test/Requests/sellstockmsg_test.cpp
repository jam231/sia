...
void SellStockTest::from_valid()
{
    ...
    try
    {
        ...
        assert (length == 15);
        assert (Message::REQUEST_SELL_STOCK_ORDER == 0x14);
        ...
        QVERIFY2(sell_stock.length() == length,
                 qPrintable(QString("Length is incorrect. Is %1 should be %2.")
                            .arg(sell_stock.length())
                            .arg(length)));

        QVERIFY2(sell_stock.getStockId() == stock_id,
                 qPrintable(QString("Stock id doesn't match. Is %1 should be %2.")
                            .arg(sell_stock.getStockId().value)
                            .arg(stock_id.value)));
        QVERIFY2(sell_stock.getAmount() == amount,
                 qPrintable(QString("Amount doesn't match. Is %1 should be %2.")
                            .arg(sell_stock.getAmount().value)
                            .arg(amount.value)));
        QVERIFY2(sell_stock.getPrice() == price,
                 qPrintable(QString("Price doesn't match. Is %1 should be %2.")
                            .arg(sell_stock.getPrice().value)
                            .arg(price.value)));
    }
...
void SellStockTest::from_invalid_body_data()
{
...
void SellStockTest::from_invalid_body()
{
...
void SellStockTest::from_malformed_data()
{
    QTest::addColumn<QByteArray>("byte_array");
...
void SellStockTest::from_malformed()
{
    QFETCH(QByteArray, byte_array);
